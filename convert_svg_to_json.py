...
def parse_path_d(d_string):
    ...
    for command, args_str in path_regex.findall(d_string):
        ...
        if command == 'M': # Moveto
            # --- FIX: Check for at least 2 arguments (x, y) ---
            ...
            if current_poly:
                if len(current_poly) > 1 and current_poly[0] != current_poly[-1]:
            ...
            for i in range(0, len(args), 7):
                # Add a check to prevent IndexError on malformed arc commands
                if i + 6 >= len(args):
                    continue # Skip this malformed arc segment
...
def main(args):
    ...
    print(f"Converting {len(svg_files)} SVG files from '{args.input_folder}' to '{args.output_folder}'...")
    ...
    print(f"IMPORTANT: Make sure the 'SCALE_FACTOR' ({SCALE_FACTOR}) is set correctly to convert coordinates to meters.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Converts SVG floorplans to JSON format.")
    parser.add_argument(
        "--input_folder", 
        type=str, 
        default='data/svg',
